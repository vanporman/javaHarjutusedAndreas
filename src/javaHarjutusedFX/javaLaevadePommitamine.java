...
public class javaLaevadePommitamine extends Application {
	...
	public static void main(String[] args) {
		...
		prindiManguLaud(lauaAlgseis); // Näitab korra lauda koos asetatud
										// laevadega
		System.out.println();
		// Näita mängijale lauda
		prindiManguLaud(lauaSeis);

		// Uus Scanner objekt, hakkame seda kasutama kasutajalt sisendi
		// saamiseks
		Scanner sc = new Scanner(System.in);

		// Mängu enda tsükkel. Töötab nii kaua kuni näeb break; käsku.
		while (true) {
			...
			if (tabamus == 1) { // Tuleb välja, et nr 1 oli seal
				...
				System.out.println("Pihtas!"); // Et mängijaga näeks inimese
												// keeles ka mis juhtus.
			} else if (tabamus == 0) { // Juhul kui oli hoopis 0 sellel
										// positsioonil.
				lauaSeis[y][x] = 3; // 3 on mööda. Seda näitame kasutajale,
									// taustlauale pole vaja märkida
			...
			boolean labi = kasMangOnLabi(lauaAlgseis); // Meetod, mis vaatab
														// peale igat tsüklit
														// laua üle ja
														// konrollib, kas laevu
														// on veel
	...
	// meetod võtab sisse maatriksi ja me nimetame selle maatriksi ümber
	// "laudLopp" muutujaks
